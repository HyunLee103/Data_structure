...
class ProductionList():
    def __init__(self, Filename):
        ...
        self.nodeHead.next = self.nodeTail
        """ method를 사용하지 않고도 간단하게 짤 수 있는데, 왜 setNextNode를 선언했을까?"""
    ...
    def addLast(self, node):
        ...
        # node of the tail
        nodeLast = self.nodeTail.prev
        nodeLast.next = node
        node.next = self.nodeTail
        node.prev = nodeLast
        self.nodeTail.prev = node
    ...
    def showPlanChart(self):
        ...
        while node is not self.nodeTail:
            node = node.next
